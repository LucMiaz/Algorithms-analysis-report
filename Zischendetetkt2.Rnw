...
\begin{figure}
<<barplot>>=
barplot<-ggplot(data=filter(tf, tf$author!='esr' & tf$quality!=1), aes(spec, fill=as.factor(disct)))+geom_histogram()+scale_fifill_brewer('Paired')+xlab("BPR") + ylab("Count")+facet_wrap(algprop~author, scale="free")
